    ...
    for param_name, param in sig.parameters.items():
        ...
        if isinstance(default, Depends):
            ...
            if not hasattr(req_obj, "_dependency_cache"):
                req_obj._dependency_cache = {}
            
            use_cache = getattr(default, "use_cache", True)
            if use_cache and dep_func in req_obj._dependency_cache:
                resolved[param_name] = req_obj._dependency_cache[dep_func]
                continue
            ...
            try:
                ...
                is_async_gen = inspect.isasyncgenfunction(dep_func) or (hasattr(dep_func, "__call__") and inspect.isasyncgenfunction(dep_func.__call__))
                is_sync_gen = inspect.isgeneratorfunction(dep_func) or (hasattr(dep_func, "__call__") and inspect.isgeneratorfunction(dep_func.__call__))
                ...
                if is_async_gen or is_sync_gen:
                    if is_async_gen:
                        ...
                        req_obj._yield_cleanups.append(async_cleanup)
                    else:
                        gen = dep_func(**dep_kwargs)
                        dep_val = next(gen)
                        def sync_cleanup():
            ...
            if is_pydantic:
                try:
                    ...
                    for err in errors:
                        loc = err.get("loc", ())
                        err["loc"] = ("body",) + loc
                    raise HTTPUnprocessableEntity(detail=errors)
            else:
